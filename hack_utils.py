...
class AlbuTransforms:
    ...
    def __call__(self, sample):
        ...
        sample['image'] = torch.as_tensor(self.normalize(transformed['image']),
                                          dtype=torch.float).permute(2, 0, 1)
        landmarks = np.array(transformed['keypoints'], dtype=np.int16).flatten()
        sample['landmarks'] = torch.as_tensor(landmarks, dtype=torch.float)
        return sample
